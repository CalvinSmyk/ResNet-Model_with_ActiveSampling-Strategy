...
class ResNet(nn.Module):
    def __init__(self,block,num_block,num_outputs=100):
        super(ResNet,self).__init__()
        self.in_planes = 64

        self.init = nn.Conv2d(3,64,kernel_size=3,stride=1,padding=1,bias=False)
        ...
        self.linear = nn.Linear(512*block.expansion, num_outputs)
    ...
    def forward(self,x):
        out = F.relu(self.bn1(self.init(x)))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out)
